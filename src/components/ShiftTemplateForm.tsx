...
const formSchema = z.object({
  ...
  duration_hours: z.coerce.number().min(0.5, { message: "Duration must be at least 0.5 hours." }).max(24, { message: "Duration cannot exceed 24 hours." }),
  default_start_time: z.string().regex(/^([01]\d|2[0-3]):([0-5]\d)$/, { message: "Invalid time format (HH:mm)." }),
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="flex justify-end space-x-2">
          ...
          <Button type="submit" disabled={isSubmitting}>
            {isSubmitting ? (initialData?.id ? "Updating..." : "Creating...") : (initialData?.id ? "Update Template" : "Create Template")}
          </Button>
        </div>
      </form>
    </Form>
